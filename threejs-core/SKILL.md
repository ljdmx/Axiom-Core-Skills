...
> Before upgrading → `docs/diagnostics.md §4` — Breaking Change Map
...
└─ Performance problem / black screen / color shift / missing shadows
    └─ docs/diagnostics.md — 30-Second Triage Guide
```
...
## 0. Pre-Flight Calibration

> **Read `../_core_axioms/SOUL_MANIFESTO.md` and `references/visual-language.md` before writing a single line of code.**
> Technical correctness ≠ visual impact. Soul comes from philosophy before implementation.
...
> ```javascript
> view_file(docs/boilerplates.md) // See §1.1
> ```

### 1.2 Scene Hierarchy

> ```javascript
> view_file(docs/boilerplates.md) // See §1.2
> ```
...
> ```javascript
> view_file(docs/boilerplates.md) // See §2.1
> ```

### 2.2 Geometry — Segment Budget

> ```javascript
> view_file(docs/boilerplates.md) // See §2.2
> ```
...
|:---|:---|:---:|:---:|:---:|
| Hero | Main subject | 0.0–0.2 | 0.7–1.0 | 1.5–2.5 |
| Supporting | Secondary objects | 0.3–0.5 | 0.2–0.5 | 0.6–1.0 |
| Background | Atmospheric fill | 0.7–1.0 | 0.0–0.1 | 0.1–0.3 |

---

## 3. Lighting System Design

> Read `docs/visual-language.md §1–2` before implementing any lights.
> **Single dominant key light = all dramatic tension. Equal multi-light = CAD model, no soul.**

### 3.1 Dominant Key Light Setup

> ```javascript
> view_file(docs/boilerplates.md) // See §3.1
> ```

### 3.2 Environment Map

> ```javascript
> view_file(docs/boilerplates.md) // See §3.2
> ```
...
function animate() {
  ...
  // ~0.25 Hz = 12–18 breaths/min = 4s cycle → reinforces Companionship axis
...
|:---|:---|:---|
| Duration | 100–300ms | Above 300ms feels sluggish; below 100ms feels mechanical |
...
> ```javascript
> view_file(docs/boilerplates.md) // See §4.1
> ```

### 4.4 useFrame in R3F

> Standard frame loop patterns are detailed in `docs/boilerplates.md` §4.4.
...
> ```javascript
> view_file(docs/boilerplates.md) // See §5.1
> ```

### 5.2 Scroll-Driven Camera Path

> ```javascript
> view_file(docs/boilerplates.md) // See §5.2
> ```

### 5.3 FOV Emotional Grammar

> FOV is emotional distance — see `docs/visual-language.md §4` for full grammar.
> ```javascript
...
> ```javascript
> view_file(docs/boilerplates.md) // See §6.1
> ```

### 6.2 @react-three/postprocessing (R3F)

> ```jsx
> view_file(docs/boilerplates.md) // See §6.2
> ```
...
|:---|:---|:---|:---|:---|
| **Desktop** | ≤ 200 | ≤ 500K | 2–3 × 2048 | Full stack |
| **Mobile** | ≤ 50 | ≤ 100K | 0–1 × 1024 | None or 1 pass |
| **Low-end** | ≤ 20 | ≤ 50K | Off | Off |

### 7.2 Core Techniques

> InstancedMesh, LOD, GC Defense, and Raycaster throttling are detailed in `docs/boilerplates.md`.
> Full diagnosis → `docs/diagnostics.md §1`

### 7.3 Quick Diagnostics

> ```javascript
> view_file(docs/boilerplates.md) // See §7.3
> ```
...
> For advanced GLSL: noise functions, SDF raymarching, holographic → `docs/advanced-shaders.md`
> For TSL Node Materials (WebGL + WebGPU) → `docs/webgpu.md §2–3`
...
> ```javascript
> view_file(docs/boilerplates.md) // See §9.0
> ```

### 9.1 GLTF Loading

> Custom center/normalize logic can be found in `docs/boilerplates.md`.

### 9.2 Loading Manager with Entry Ritual

> ```javascript
> view_file(docs/boilerplates.md) // See §9.2
> ```
...
| 100K GPU particles | WebGPU | Compute Shader + StorageBuffer | docs/webgpu.md §4 |
| Holographic material | WebGPU | TSL NodeMaterial + Fresnel node | docs/webgpu.md §3 |
| Physics sandbox | R3F | @react-three/rapier + RigidBody | docs/physics.md |
| AR product trial | R3F | WebXR + @react-three/xr | docs/mobile-xr.md |
| SDF raymarcher | Vanilla | Fullscreen ShaderMaterial | docs/advanced-shaders.md §3 |
