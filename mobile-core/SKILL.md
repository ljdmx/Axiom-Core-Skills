...
token_budget:
  core_load: 5300       # SKILL.md only (must load first)
  full_load: 29000      # Full load ceiling (must AVOID)
...
## 0️⃣ Guardian Boot Protocol — MUST execute before every generation
...
> Full code for 5-sense synergy (tactile/auditory semantics) and Aura Diffusion background engine can be found in `docs/design.md` §4-§5.
...
- **Web3 Silent Tolerance**: If the user project explicitly involves Web3 features (e.g., wallet connection) while in standalone mode, AI MUST flexibly `view_file(../web3-core/docs/frontend-wagmi.md)` to borrow key integration rules rather than hard-failing or waiting for full FSPC orchestration.
...
>
> If any condition fails → Exit FSPC-Driven Mode, run standard standalone Guardian Boot Steps 1–5.

When in **FSPC-Driven Mode**, skip Guardian Boot Steps 1–4 and execute this compressed entry instead:
...
### 0.1–0.4 Soul Derivation & Audit Core (High-Density)
...
|------|-------------|-----------|
| **Requires HBuilderX** | Required for packaging | Required for Dev + Packaging (≥3.9)|
...
> Full process in `docs/publishing.md`
...
> **Full Design System in `docs/design.md`**  
...
| **Physical Gravity & Elasticity** | Animations and scroll bounds MUST map to physical world mass and spring dynamics. Define precise cubic-bezier curves for rubber-band overscroll effects. Linear or stiff easing is a hard violation of mobile native feel. |
...
| **Haptic Companionship** | For all critical physical interactions (e.g., submitting a payment, deleting an item, toggling a switch, or pull-to-refresh), AI MUST automatically wire up `uni.vibrateShort()` to provide subtle tactile feedback. This completes the illusion of life and physical reality. |
| **Unified Iconography** | NEVER mix icon families. You MUST strictly use exactly ONE premium vector icon library (e.g., Phosphor Icons or Lucide) across the entire project. All icons MUST have identical thickness and style. Reject native emojis or mis-matched SVGs. |
...
|---------|---------|------------------------|
| uni-app Classic · Vue3 · State Management · uniCloud | `docs/classic.md` | uni-app classic, Vue3, mini-program, H5, Pinia, uniCloud |
| uni-app x · UTS · uvue Native Rendering | `docs/uni-app-x.md` | uni-app x, UTS, uvue, native, Kotlin, ArkTS |
| Design Token · Colors · Typography · Animations · Skeleton | `docs/design.md` | design system, Token, animation, color, dark mode, skeleton, typography |
| Materials/Lighting · Widescreen/Responsive · Web A11y | `docs/design-advanced.md` | material, lighting, tablet, foldable, performance budget, accessibility, A11y |
| Advanced Components · Headless · Physical Feedback | `docs/components.md` | components, navbar, FAB, popup, form, input |
| Multi-platform Publishing · CI/CD · Certificate Mgmt | `docs/publishing.md` | publishing, cloud package, certificates, AppStore, Google Play, CI/CD |
...
| **v5.3** | 2026-03 | Renamed references/ to docs/, added global nexus protocol |
