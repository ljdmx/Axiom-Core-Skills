 ...
 *   5. docs/ directory presence and non-empty
...
for (const skill of SKILLS) {
  ...
  // --- 4. Stale references/ paths ---
  const staleRefs = (skillContent.match(/`references\//g) || []).length;
  const staleLinks = (skillContent.match(/\(references\//g) || []).length;
  if (staleRefs + staleLinks === 0) {
    pass('No stale references/ paths found');
  } else {
    error(`Found ${staleRefs + staleLinks} remaining references/ path(s) — update to docs/`);
  }
  ...
  // --- 6. docs/ directory not empty ---
  const docsDir = path.join(skillDir, 'docs');
  if (fs.existsSync(docsDir)) {
    const docFiles = fs.readdirSync(docsDir).filter(f => f.endsWith('.md') || f.endsWith('.json'));
    if (docFiles.length > 0) {
      pass(`docs/ directory has ${docFiles.length} file(s)`);
    } else {
      warn('docs/ exists but is empty');
    }
  } else {
    info('No docs/ directory (may be intentional for lean skill packages)');
  }
