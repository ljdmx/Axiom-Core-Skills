...
> **Status**: ACTIVE — Reference when creating or auditing a project's design token system.
> **Authority**: This document is the canonical naming standard for all DDFM-generated token systems.

---

## 🏗️ Token Hierarchy (3 Layers)

```
Layer 1: Primitive Tokens   → Raw values, named for what they ARE
Layer 2: Semantic Tokens    → Named for what they DO (reference Primitives)
Layer 3: Component Tokens   → Named for specific component usage (reference Semantics)
```
...
| `delight` | Reward moments only | `--color-delight` (use sparingly per §28) |
...
--spacing-2:   8px;    /* sm */
--spacing-3:   12px;
--spacing-4:   16px;   /* md — default base */
--spacing-5:   20px;
--spacing-6:   24px;   /* lg */
--spacing-8:   32px;   /* xl */
--spacing-10:  40px;
...
## 🪟 Border & Shadow Token Naming Convention
...
--radius-lg:   12px;
--radius-xl:   16px;
--radius-full: 9999px;  /* Pill shape — use sparingly */
...
/* Rule: shadow-depth MUST increase monotonically card → hover → modal → overlay */

/* Dark mode inner highlight (per §7) */
--shadow-inner-highlight: inset 0 1px 0 rgba(255,255,255,0.08);
```

---

## 🔢 Z-Index Token Naming Convention (5 Layers Max)
...
|---|---|---|
| v1.0 | 2026-03-09 | Initial creation — complete 3-layer hierarchy, naming conventions for color/spacing/typography/shadow/z-index, stylelint enforcement |
