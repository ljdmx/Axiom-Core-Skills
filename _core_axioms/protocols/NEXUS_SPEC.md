...
4. **Token Fuse (Hard Limit)**: If `accumulated_tokens` approaches the `hard_token_limit`, the AI MUST forcibly interrupt the generative process, cease reading extensive documentation, and switch to outputting minimal architecture snippets and bullet points to prevent token exhaustion.
5. **Architectural Self-Evolution**: After finalizing the session, the orchestrating AI MUST write to `../evolution_log.md` detailing which core axioms or skill protocols were triggered most frequently, and log any encountered bottlenecks. This ensures the ecosystem continuously self-optimizes across sessions.
