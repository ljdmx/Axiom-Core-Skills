...
4. **Accessibility (A11y) Voice Map**: All primary interactive components MUST be tagged with `aria-label` or semantic screen-reader text to establish a definitive VoiceOver navigation map.
...
> Full code for 5-sense synergy (tactile/auditory semantics) and Aura Diffusion background engine can be found in `references/mobile-design.md` §4-§5.

### Step 2: The Rejection Gate
AI MUST execute the formal Rejection Gate protocol defined in `protocols/STANDARD_GATES.md`.
...
| **0.3 Sovereign Component** | Avoid strong coupling with routing & global Store (Fail-Safe) | Components build their own state machines. Maintain micro-interactions under network loss or route freeze without dying. AI MUST wireframe/mock an "Offline-first" or explicit "2G/3G Degraded Network" optimistic UI edge-case as a Hard Block before passing UI. |
| **0.3.1 Memory Sovereignty** | Prevent Cross-mode Memory Leaks | Mandate strict Component Lifecycle Teardown. All event listeners, timers, and active canvas contexts MUST be explicitly destroyed in `onUnload` or `onBeforeUnmount`. |
| **0.3.2 Physical Safe Area** | Enforce Notch and Home Indicator padding | Absolute injection mandate for physical device safe zones. Any fixed bottom-bar or full-screen layout MUST explicitly utilize `padding-bottom: env(safe-area-inset-bottom);` or face an automatic Hard Block rejection. |
| **0.3.3 Micro-Screen Scalability** | Wearable/IoT Readiness | All core application interfaces MUST preserve foundational operability when scaled down to smartwatch or IoT panel dimensions. Employ extreme minimalist distillation for primary actions. |
| **0.3.4 DOM Depth Execution Gate**| Device Performance Guardian | To prevent fatal main-thread blocking on low-end devices, the HTML/Vue template DOM hierarchy MUST NOT exceed a maximum depth of 14 layers. Furthermore, any iterative data array anticipated to exceed 50 entries MUST be strictly rendered using a Virtual List (Windowing) architecture. |
| **0.3.5 Token Drift Prevention** | Visual Degradation Control | AI MUST run a structural Token Diff to ensure no hardcoded colors or sizing bypass the approved mobile CSS layer mappings. |
| **0.4 Aesthetic Alignment** | Reject: Symmetrical grids, stiff animation, crowded typography | **Require**: Asymmetrical Bento whitespace, Cubic Bezier press physics, extreme weak highlights/transparent noise, editorial negative-spacing titles, Strict Design Tokens (HSL Dynamic Gradients, Soft UI Depth layers, Glassmorphism). |

> **Mandatory Audit**: Before delivering the final package to the user, you MUST run `node tools/audit-skill.js [target_path]` to audit the page flow. AI MUST invoke `node tools/path_resolver.js` and universally rely on relative path references for file operations across environments.
...
### JSON Configuration Templates (pages.json, manifest.json, theme.json)
> **[TOKEN OPTIMIZATION]** 
> To reduce context payload during component evaluation:
> **AI MUST load:** `view_file(references/config-templates.md)` for standard JSON scaffolding (TabBars, generic dark mode bindings).
...
> **Full Design System in `references/mobile-design.md`**  
...
| **Physical Gravity & Elasticity** | Animations and scroll bounds MUST map to physical world mass and spring dynamics. Define precise cubic-bezier curves (strictly 200-300ms duration) for rubber-band and micro-interaction effects. Linear or stiff easing is a hard violation of mobile native feel. **Integration**: Recommend `Auto Animate` (FormKit) for frictionless list transitions. |
| **§5 Kinetics** | Mandatory 200-300ms bezier curve transitions or hardware-accelerated Spring Animations (e.g. React Native Reanimated `withSpring({ damping: 10, stiffness: 100 })`). **NO simple linear mobile transitions.** |
| **§6 Haptics** | MUST implement Haptic Feedback (`Haptics.impactAsync('light')` or platform equivalent) mapped strictly to user actions (e.g. success, error, list end bounce) to ensure the client feels alive and reactive, surpassing basic "web-apps in a wrapper". |
...
### 5.2 Zenith Ascension: Peer Dependency Shield & Pre-Flight Diagnostics
- **Zero-Error Initialization (Sandbox Init)**: Before generating any `.vue` or `.uvue` files, AI MUST run a relative path script (e.g., `node ../backend-core/tools/sandbox-init.js` or `sh ./scripts/verify-mobile-env.sh`) to assert dependencies, quietly install missing CLI tools (e.g., `vite`, `sass`), and verify the presence of HBuilderX paths if required.
- **Standalone Mode Capability**: Ensure that if `product-core` or `PROJECT_NEXUS.json` is missing, `mobile-core` degrades gracefully to standalone generation without failing, utilizing internal minimalist templates to guarantee a zero-error first run.
- **Fail-Fast Haptic & Ergonomic Breakpoint**: During generation, if ANY primary interactive component (like a payment button or critical toggle) is detected without a `uni.vibrateShort()` tactile binding or falls outside the bottom 40% thumb zone, AI MUST immediately throw a `[Mobile Ergonomic Error]` and self-heal the component on the spot before continuing.
- **Native Linkage Autonomy**: Mobile environments (like Expo or React Native) are exceptionally fragile. AI MUST autonomously execute peer-dependency audits and native link checks via local relative scripts (e.g., `node ./scripts/mobile-audit.js`).
- **Self-Healing Cycle**: AI MUST self-heal any native module conflict, pod installation failure, or Gradle sync error instantly before deploying the UI payload to the user. Do not wait for manual user escalation.

### 5.3 Zenith Ascension II: Mechanical Sovereignty & Render Limits
- **Virtual DOM Mandate**: To prevent fatal main-thread throttling on mid-tier mobile hardware, any iterative array structure forecasted to exceed 50 elements MUST strictly implement a Virtual Window/List architecture (e.g., `uc-virtual-list`). Standard loops for large datasets are a HARD BLOCK.
- **Haptics-as-Data**: AI MUST NEVER manually infer vibration curve logic. All tactile feedback mappings MUST be externalized to a deterministic `./config/haptics-matrix.json`. AI must bind user interactions strictly against this predefined physical feedback map to ensure native parity.

### CSS Cross-Mode Compatibility Strategy

> **[TOKEN OPTIMIZATION]** 
> To reduce Context Load during cross-platform evaluations:
> **AI MUST load:** `view_file(references/syntax-dictionary.md)` for detailed uni-app Classic vs uni-app x CSS subset comparisons.
...
> **[TOKEN OPTIMIZATION]** 
> The full table of `APP-PLUS`, `H5`, `MP-WEIXIN`, and `HARMONY` compilation flags has been extracted.
> **AI MUST load:** `view_file(references/syntax-dictionary.md)` if confused about platform flags.
...
|---------|---------|------------------------|
| uni-app Classic · Vue3 · State Management · uniCloud | `references/classic-patterns.md` | uni-app classic, Vue3, mini-program, H5, Pinia, uniCloud |
| UTS Native · uni-app x · Swift · Kotlin              | `references/uts/uni-app-x.md`  | uts, uni-app x, native, swift, kotlin, performance       |
| Design Token · Colors · Typography · Animations · Skeleton | `references/mobile-design.md` | design system, Token, animation, color, dark mode, skeleton, typography |
| Materials/Lighting · Widescreen/Responsive · Web A11y | `references/mobile-design-advanced.md` | material, lighting, tablet, foldable, performance budget, accessibility, A11y |
| Advanced Components · Headless · Physical Feedback | `references/components.md` | components, navbar, FAB, popup, form, input |
| Unified Mobile Iconography Standards           | `references/mobile-icon-guidelines.md` | icons, phosphor, sizing, color matching, scaling ratio |
