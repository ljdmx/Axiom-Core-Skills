...
> This file is an advanced extension of `mobile-design.md`. Condition for loading: Requirements involve ONE of the TRIGGER keywords above.
> For core Design Tokens, Colors, Typography, and Animation Systems, please see `references/mobile-design.md`.
...
## 7. Advanced Haptics & Spring Physics Engine (Native Parity Zenith)

### 7.1 Deterministic Haptic Mapping
Relying on random vibration is amateurish. High-order apps MUST map specific device haptics (`uni.vibrateShort()`) to standard operational physics:
- **Light (`type: 'light'`)**: UI state changes (Switches, Tab bar navigation, Dropdown selections, Pull-to-refresh tension).
- **Medium (`type: 'medium'`)**: Context confirmations (Save button pressed, Modals/Drawers opening).
- **Heavy (`type: 'heavy'`)**: Destructive actions or Error triggers (Delete item, Form validation failure).

### 7.2 Cubic-Bezier Spring Dynamics
Linear transitions (`ease`) feel robotic. Mobile UI MUST employ physical spring equations reflecting mass and tension.
- **Rubber Band (Overshoot)**: Action sheets and drawn-over lists MUST overshoot and settle: `cubic-bezier(0.34, 1.56, 0.64, 1)` representing tension.
- **Air Resistance (Deceleration)**: Swiping away elements should use `cubic-bezier(0.16, 1, 0.3, 1)` mimicking friction over time.
- Implementation: `transition: transform 300ms cubic-bezier(0.34, 1.56, 0.64, 1);`
