...
## 7.4 Zenith Ascension: Lighting Determinism & Environment Sanity Check
- **Zero-Error Initialization (Sandbox Init)**: AI MUST execute a deterministic sandbox check script (e.g., `node ../backend-core/tools/sandbox-init.js` or `sh ./scripts/verify-3d-env.sh`) prior to scaffolding any Three.js project to assert module presence (like `@react-three/fiber` or `three` versions) and silently install missing packages.
- **Standalone Mode (Fallback)**: If global context (`product-core`, `_core_axioms`) is absent, AI MUST degrade gracefully to run completely independent using internal `templates/init-scene.js`. Never fail execution due to missing external wrappers.
- **Zero-Blackout Guarantee**: Rendering a scene with uninitialized lighting resulting in an indistinguishable black shape is a catastrophic failure. 
- **Deterministic Illumination**: AI MUST load deterministic base environment maps (HDRI) from local relative paths (e.g., `./assets/environments/studio.hdr`) by default globally. 
- **Fail-Fast WebGL Assertions & Material Self-Healing**: If a texture path is missing or a custom Shader fails compilation, the scene MUST NOT crash to a black screen. AI MUST implement a strict WebGL Context Assertion: any failing material must immediately trigger a `[WebGL Error] Breakpoint`, halting further scene generation, while autonomously swapping the broken material with a basic colored `MeshBasicMaterial` fallback. A lightweight sub-task must instantaneously fix the Shader or path issue before resuming.

## 7.5 Zenith Ascension II: Deterministic Illumination Rigs
...
- **Parameter Blind-Guessing Ban**: Creating dramatic 3D environments by manually nudging `DirectionalLight` coordinates is strictly forbidden. 
- **World-Class Lighting Rigs**: AI MUST exclusively import mathematically perfect, pre-calibrated lighting environments from local relative components (e.g., `import { CyberpunkNeonRig, CinematicStudioRig } from './templates/lighting-rigs.js'`). This enforces flawless, zero-blackout aesthetic rendering instantly.
- **Aesthetic Automation Grading**: AI MUST utilize `node tools/aesthetic-scorer.js` tailored for 3D (shadow map resolution, light contrast ratios) and ensure a score >= 90 before handoff.

## 8. GLSL Shader Protocol

### 8.1 Shader Rules & Vanilla Templates
> **[TOKEN OPTIMIZATION]**
> To reduce context payload:
> **AI MUST load:** `view_file(references/shader-rules.md)` for Vanilla GLSL Boilerplate, Lightmap Baking, and World-Class PBR Shading rules.

> **Shader Linter Gate (Zenith Protocol)**: Before rendering any custom GLSL, AI MUST execute `glslangValidator` or a corresponding JS-based GLSL linter within the `./templates/` pipeline. Any Shader compilation error must be self-healed before passing to the Canvas.
> **Asset Verification Protocol**: AI MUST generate and validate against `./assets/manifest.json` using `node tools/asset-verifier.js` to prevent 404 texture/model loads breaking the WebGL context.
