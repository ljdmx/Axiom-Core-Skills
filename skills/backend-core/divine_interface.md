...
> **Status**: ACTIVE â€?Provides structured handling protocols for non-text inputs.
> These are AI behavioral instructions, not executable code.

---

## ðŸ–¼ï¸?Visual Input Processing (Architecture Diagrams, Whiteboard Photos)
...
3. **Determine Root** â€?apply ADBM Â§5 Aggregate Root Analysis: which entity has no inbound foreign-key arrows? That is the Aggregate Root.
4. **Output Mermaid `classDiagram`** (ADBM Â§5 mandatory format) before generating any schema code.
5. **Audit** for missing ADBM Â§5 mandatory fields: `id` (UUIDv7), `created_at`, `updated_at`, `created_by`; add them to all entities.
...
## ðŸŽ™ï¸?Audio / Transcript Input Processing

**Trigger**: User provides a voice transcript or describes a system verbally.

**AI MUST execute in this sequence**:

1. **Extract nouns** â†?candidate Core Entities.
2. **Extract verbs** â†?candidate business operations. Map to ADBM Â§4 Anti-Naked-CRUD verbs (e.g., "user places order" â†?`POST /orders/place`, not `POST /orders`).
3. **Detect scale signals** in language:
   - "millions of users", "real-time data" â†?escalate to `Scale = Growth/Enterprise`
   - "just us 3", "weekend project" â†?`Scale = Startup`
4. **Run FSPC PRE-FLIGHT Domain Interrogation** if key entities are unclear or entity count < 5.
5. Output ADBM/FSPC Â§0 Classification Tag before proceeding.
...
## ðŸŒ URL / Screenshot Input Processing (Clone / Reverse-Engineer)

**Trigger**: User provides a URL or screenshot of an existing product to clone or reference.

**AI MUST execute in this sequence**:

1. **Infer entity model** from visible UI elements: forms â†?input fields â†?entity attributes; tables â†?list views â†?entity relationships.
2. **Apply ADBM Â§5 DDD reverse-engineering**: identify which visible data objects are Aggregate Roots vs. child entities.
3. **Output a PRD draft** (`PRODUCT_SPEC.md` skeleton) for user confirmation before generating any code.
4. **Do NOT copy** proprietary business logic â€?focus on structural patterns only.

**Output Sequence**:
```
1. [Inferred entity model as Mermaid classDiagram]
2. [PRODUCT_SPEC.md skeleton]
3. [User confirmation gate â€?await "Approve" before proceeding]
```
...
| v1.0 | 2025-Q4 | Initial stub â€?Technical Specification only, no behavioral guidance |
