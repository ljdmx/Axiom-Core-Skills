...
async function runConfigWizard() {
    log('\n🚀 Smart Configuration Manager (ADBM-Aligned)', 'cyan');
    log('━'.repeat(50), 'cyan');
    ...
    if (usePostgres && !hasPostgres) {
        log('\n⚠️  PostgreSQL not detected. Install from https://www.postgresql.org/download/', 'yellow');
    }
    if (!usePostgres && !hasMysql) {
        log('\n⚠️  MySQL not detected. Install from https://dev.mysql.com/downloads/mysql/', 'yellow');
    }

    // Database Configuration
    const defaultPort = usePostgres ? '5432' : '3306';
    const defaultUser = usePostgres ? 'postgres' : 'root';
    log(`\n🗄️  ${usePostgres ? 'PostgreSQL' : 'MySQL'} Configuration`, 'blue');
