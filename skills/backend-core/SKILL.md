...
- **Hyper-Realistic Data Constraints**:
  ...
  3. **Data Volume & Distribution**: Seed data MUST represent a realistic usage curve (e.g., 20% power users with 500+ items, 60% average users with 10�?0 items, 20% new users with < 5 items). A flat distribution is not realistic.
...
| v8.1 | 2026-03-09 | §11�?2 (Contract Testing/GraphQL), §14�?5 (DLQ/LLM), §19�?0 (Sync/Privacy) extracted to `references/backend-rules-advanced.md` (saves ~6K tokens). Fixed duplicate §26 �?Multi-Tenant Migration Ladder renamed to §27. Expanded `red_team/security_audit.ts` to full OWASP Top 10 coverage. Added smart-config.js trigger. Created `references/adr-template.md` and `references/tech-debt-template.md`. |
