...
const AXIOMS_DIR = path.join(SKILLS_ROOT, '_core_axioms');
...
for (const skill of SKILLS) {
  ...
  // --- 8. SKILL_PRIORITY & Conflict Triage ---
  if (skill === 'product-core') {
      const priorityPath = path.join(skillDir, 'SKILL_PRIORITY.md');
      if (fs.existsSync(priorityPath)) {
          pass('SKILL_PRIORITY.md (Conflict Triage) present in product-core');
          const priorityContent = fs.readFileSync(priorityPath, 'utf8');
          if (priorityContent.includes('Global Priority Hierarchy')) {
              pass('Conflict Arbitration matrix is valid');
          } else {
              error('SKILL_PRIORITY.md missing Global Priority Hierarchy matrix');
          }
      } else {
          error('SKILL_PRIORITY.md missing — required for automated conflict triage');
      }
  }

  // --- 9. Nexus Compliance ---
  if (fs.existsSync(AXIOMS_DIR)) {
      const nexusSpec = path.join(AXIOMS_DIR, 'NEXUS_SPEC.md');
      if (fs.existsSync(nexusSpec)) {
          if (skillContent.includes('PROJECT_NEXUS.json')) {
              pass(`Nexus Protocol integration detected in ${skill}`);
          } else if (skill !== 'product-core') {
              warn(`${skill} should reference PROJECT_NEXUS.json for context continuity`);
          }
      }
  }
}
