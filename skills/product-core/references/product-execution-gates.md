...
- **Execution**: Deliver `PRODUCT_SPEC.md` and `references/adr/` (Architecture Decision Records).
...
## 1️⃣6️⃣ Pivot Intelligence Protocol (Anti-Stagnation)
> **Sovereign Pivot Intelligence**
> Scope: When technology stacks block progress repeatedly, AI MUST propose an architectural pivot.

If the same technology (framework / DB / deployment target) causes **≥ 3 consecutive build failures**, blocks progress for **> 30 continuous minutes**, OR if the DDFM Aesthetic Score remains **< 7.0 for 3 consecutive iterations** (Aesthetic Stagnation), AI MUST proactively output a `PIVOT_PROPOSAL.md` containing:
1. Current blockers, error traces, or aesthetic stagnation details.
2. Proposed alternative stack (e.g., NestJS -> FastAPI, CRA -> Vite).
3. Estimated migration cost in hours.
4. An explicit request for user `Approve` before switching.

This protocol ensures the Agent does not get stuck in infinite looping fixes for fundamentally broken architectures or incompatible library versions.

## 0️⃣ Axiom Core v2.1: FSPC Pre-Flight Protocol (Phase 0)

### Phase 0A — Domain & Persona
> **Mandate**: Business Intelligence First.
- **Role x Permission Matrix**: Mandatory for multi-user systems.
- **Entity Threshold**: Model depth check (Min 5 entities).
- **User Journey Mapping**: Role-based flow diagrams.
- **PRD Sign-off**: Using `./templates/product-spec.template.md`.

### Phase 0B — Architecture & Monetization
- **Tenant Isolation**: Shared Schema / Separate Schema / Separate DB.
- **Architecture Brief**: Tech stack decision document.
- **FinOps Estimation**: Monthly cost profiles based on RPS.
- **Value Capture**: Declare monetization strategy (SaaS/Marketplace/API).

### Phase 0C — Engineering Scaffold
- **Scaffold Logic**: CI/CD initialize + Husky + Commitlint.
- **Red Team Security Sync**: Automated vulnerability scan.
- **Spec-First API**: OpenAPI generation before code.

### Phase 0D — Phase-Locked State Machine
- **Backend First**: Schema + API verification before UI boot.
- **handshake logic**: Phase 1.5 Deep Asset Generation for 3D/Web3.
