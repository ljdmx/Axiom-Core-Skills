...
## 0ï¸âƒ£ Tokenomics Inversion Layer [Inversion]
...
## ðŸ§˜ Security First: The Forge Protocol
...
## ðŸ—‚ï¸?Routing by Project Type

**NFT Project (PFP / Art / Game Items)**
â†?`nft.md` + `security.md Â§1,Â§6` + `deploy.md Â§Verification`

**DeFi Protocol (DEX / Lending / Yield)**
â†?`defi.md` + `solidity-patterns.md Â§ERC-4626` + `security.md Â§4,Â§5` + `testing.md Â§fork`

**dApp Frontend (React/wagmi)**
â†?`frontend-wagmi.md` + `data-infra.md`

**Gasless / AA Experience**
â†?`account-abstraction.md` + `frontend-wagmi.md Â§5 AA`

**Multi-chain / L2 Deployment**
â†?`l2-deployment.md` + `deploy.md Â§Multi-chain`

**Cross-chain Protocol**
â†?`crosschain.md` + `crosschain.md Â§4 Cross-chain Security`

**Security Audit / Code Review**
â†?`security.md` (Full) + `toolchain.md Â§Slither`

**New Project from Scratch**
â†?`deploy.md Â§Framework Selection` + `toolchain.md Â§CI-CD` + `solidity-core.md`

**Full DApp (Contract + Backend + Frontend) â€?FSPC Dispatch Required**
â†?This scope exceeds web3-core alone. AI MUST recommend activating `product-core` (FSPC) as the orchestrator:
...
1. **Security First** â€?CEI Pattern + `nonReentrant`; sensitive functions MUST have access control.
2. **Gas Efficiency** â€?Storage operations are the most expensive; prefer `calldata`; make good use of `immutable` and custom `error`.
3. **Standard Priority** â€?OpenZeppelin v5; do not reinvent the wheel.
4. **Brand & UI Accuracy** â€?Mandate **Phosphor Icons v2.1** for all general UI elements to maintain cross-ecosystem consistency. Use **Simple Icons** EXCLUSIVELY for Coin, Chain, and Blockchain Brand logos.
5. **Dependency Version Confirmation** â€?At the START of every session, AI MUST run:
   ```sh
...
5. **Test Coverage** â€?Unit + Fuzz + Invariant + Fork; Coverage > 95%.
6. **Type Safety** â€?TypeChain or wagmi CLI codegen to auto-generate ABI bindings.
7. **Explicit Versions** â€?Code MUST annotate OZ version (v4/v5), wagmi version (v1/v2) in comments.
8. **Optimistic Projection (DeFi UX)** â€?Any state-mutating transaction MUST trigger an Optimistic UI state on the frontend *before* block confirmation, coupled with a defined 'waiting for network' companion animation to eliminate user anxiety.
9. **Visual Delta Simulation (Anti-Blind-Signing)** â€?Hard requirement for pre-flight transaction simulation. Users MUST be shown exact balance changes (in fiat/token amounts) using **Visual Delta** displays (e.g. Magic UI Shimmer/Beam effects) to provide human-readable intent BEFORE they press "Sign/Confirm".

---
## ðŸŒ Federated Handoff & Global Nexus Protocol
...
1. **State Injection â€?web3_context Write-Back**: After contract design is finalized, AI MUST write to `PROJECT_NEXUS.json.sub_skill_outputs.web3_core`:
```json
{
  "active": true,
...
> Full attack patterns in `references/security.md`. Before delivering the contract, you **MUST** run `node tools/audit-skill.js` for baseline auditing. If it triggers an ERROR, you must run a deep scan with Slither first.
...
|---|---|---|
| v3.1 | 2026-03-09 | Renamed references/ to references/ for OS unification. Added Global Nexus Protocol. Added web3_context write-back. Added Full DApp FSPC routing. Added Dependency Version Confirmation mandate. |
