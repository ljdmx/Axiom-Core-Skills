...
## Contract Testing · GraphQL · DLQ Governance · LLM Protocol · Local-First · Data TTL

> **Status**: ACTIVE — Loaded on-demand by ADBM SKILL.md.
> **Load Triggers**:
> - §11 Contract Testing → any project with service-to-service API dependencies
> - §12 GraphQL → project uses GraphQL schema
> - §14 DLQ → project uses message queues
> - §15 LLM → project calls an LLM/AI model API
> - §19–§20 → consumer/B2C apps with strong privacy requirements
> **Usage**: `view_file(docs/backend-rules-advanced.md)`
...
- **Mutation Testing**: For critical payment and auth endpoints, integrate `stryker` or equivalent mutation testing to validate that tests genuinely catch business logic regressions — not just code coverage percentages.
...
- **Schema Governance**: All schema changes MUST go through a `schema.graphql` diff review in CI. Breaking changes (removing fields, changing types) require a deprecation period matching §1 versioning rules. Use `graphql-inspector` in CI to automate breaking-change detection.
- **Subscription Backpressure**: Any GraphQL subscription with real-time data streams MUST implement server-side backpressure — do not push raw DB change events faster than clients can process.
...
- **Fallback Chain**: Define a fallback model chain for production resilience: `primary_model → fallback_model → cached_response → graceful_degradation_message`. Never let an LLM API outage white-screen the user.
...
- **Conflict Resolution**: When two clients modify the same resource concurrently, use CRDT (Conflict-free Replicated Data Types) or Last-Write-Wins with vector clocks — never silently overwrite either client's work.
...
- **Audit Trail Exception**: Audit logs and financial transaction records have separate legal retention requirements (typically 5–7 years). These MUST be exempted from TTL-based deletion and documented in the Data Retention Policy.
...
|---|---|---|
| v1.0 | 2026-03-09 | Extracted from ADBM SKILL.md §11, §12, §14, §15, §19–§20. Enhanced each section with additional rules: mutation testing, GraphQL schema governance, DLQ dashboard mandate, LLM cost observability + fallback chain, CRDT conflict resolution, GDPR erasure propagation |
