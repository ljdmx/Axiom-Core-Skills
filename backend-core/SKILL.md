...
auto_activate: true
token_budget:
  core_load: 7600       # SKILL.md only (must load first)
  full_load: 36000      # Full load ceiling (with all docs/ sections)
---

# API-Driven Backend Manifesto (ADBM)
> **MUST READ**: `view_file(docs/domain-infra/protocols.md)` — RFC 2119 Rules and Core Protocols.
...
| **§8 Observability** | JSON Trinity | Structured JSON Logs + OpenTelemetry Traces + Prometheus Metrics. P1/P2 alerting rules. |
...
>
> ```
> view_file(docs/domain-auth/backend-rules-advanced.md)
> ```
...
1. **Routing & Health**: Use `blueprints/` and `red_team/security_audit.ts`. **Mandatory Audit**: Run `node tools/audit-skill.js [target_path]` before handoff.
...
| v8.1 | 2026-03-09 | §11–12 (Contract Testing/GraphQL), §14–15 (DLQ/LLM), §19–20 (Sync/Privacy) extracted to `docs/backend-rules-advanced.md` (saves ~6K tokens). Fixed duplicate §26 — Multi-Tenant Migration Ladder renamed to §27. Expanded `red_team/security_audit.ts` to full OWASP Top 10 coverage. Added smart-config.js trigger. Created `docs/adr-template.md` and `docs/tech-debt-template.md`. |
